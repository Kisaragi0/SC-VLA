...
class BaseDataConfig(ABC):
    def modality_config(self) -> dict[str, ModalityConfig]:
        ...
        progress_modality = ModalityConfig(
            delta_indices=self.prog_indices,    
            modality_keys=self.progress_keys,
        )
...
class PANDADataConfig(BaseDataConfig):
    ...
    def transform(self):
        transforms = [
            ...
            # progress transforms
            StateActionToTensor(apply_to=self.progress_keys),
            StateActionTransform(
                apply_to=self.progress_keys,
                normalization_modes={key: "min_max" for key in self.progress_keys},
            ),
            ...
            ConcatTransform(
                video_concat_order=self.video_keys,
                state_concat_order=self.state_keys,
                progress_concat_order=self.progress_keys,
                action_concat_order=self.action_keys,
            ),
            GR00TTransform(
...
class ARX5DataConfig(BaseDataConfig):
    video_keys = ["video.image","video.wrist_image"]
    state_keys = ["state.position","state.rotation","state.gripper"]
    action_keys = ["action.position","action.rotation","action.gripper"]
