...
class TrainRunner:
    def __init__(
        ...
        if self.rank == 0:
            ...
            elif train_dataset.__class__.__name__=='MultiDatasetWrapper':
                metadata_json = {}
                if os.path.exists(self.exp_cfg_dir / f"metadata.json"):
                    with open(self.exp_cfg_dir / f"metadata.json", "r") as f:
                        metadata_json = json.load(f)
                metadata_json = {}
                for i in range(len(train_dataset.datasets)):
                    metadata_json.update(
                        {train_dataset.datasets[i].tag: train_dataset.datasets[i].metadata.model_dump(mode="json")}
                    )
