...
class Gr00tPolicy(BasePolicy):
    ...
    def pad_state(self, data: dict, state_horizon: int = 3) -> dict:
        """
        Pad state-related entries to a fixed temporal horizon.
        """
        padded = data.copy()

        # Pad all keys starting with "state."
        state_keys = [k for k in padded.keys() if k.startswith("state.")]
        for k in state_keys:
            v = padded[k]
            v = np.array(v, dtype=np.float32)
            if v.ndim == 2 and v.shape[0] == 1:
                zeros = np.zeros((state_horizon - 1, v.shape[1]), dtype=np.float32)
                padded[k] = np.concatenate([v, zeros], axis=0)
            elif v.ndim == 3 and v.shape[1] == 1:
                zeros = np.zeros((v.shape[0], state_horizon - 1, v.shape[2]), dtype=np.float32)
                padded[k] = np.concatenate([v, zeros], axis=1)
    ...
    def get_action(self, observations: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # # obs_copy = self.pad_state(obs_copy, state_horizon=getattr(self, "state_horizon", 3))
        ...
        normalized_action,normalized_progress,normalized_delta_state = self._get_action_from_normalized_input(normalized_input)
        unnormalized_data = self._get_unnormalized_action(normalized_action, normalized_progress,normalized_delta_state)

        if not is_batch:
            unnormalized_data = squeeze_dict_values(unnormalized_data)
        return unnormalized_data

    def _trim_future_deltas(self, delta_indices: np.ndarray) -> np.ndarray:
        """
        Remove future offsets (> 0) from delta_indices.
        Examples:
            [0, 15] -> [0]
            [-1, 0, 5, 15] -> [-1, 0]
        """
        if delta_indices is None:
            return None
        return np.array([d for d in delta_indices if d <= 0], dtype=int)
    ...
    def _get_action_from_normalized_input(self, normalized_input: Dict[str, Any]) -> torch.Tensor:
        ...
        normalized_progress = model_pred["progress_pred"].float()
        normalized_delta_state = model_pred["delta_state_pred"].float()
        return normalized_action,normalized_progress,normalized_delta_state

    def _get_unnormalized_action(self, normalized_action: torch.Tensor, normalized_progress: torch.Tensor, normalized_delta_state: torch.Tensor) -> Dict[str, Any]:
        """
        Convert normalized action-related tensors back to unnormalized space.
        """
        # Construct normalized input dict
        normalized_data = {
            "action": normalized_action.cpu(),
            "progress.value": normalized_progress.cpu(),
            "delta_state": normalized_delta_state.cpu(),     
        }
        
        # Apply inverse transforms
        unnormalized_data = self.unapply_transforms(normalized_data)
        
        # Check for decomposed state components
        has_pos = "state.position" in unnormalized_data
        has_gripper = "state.gripper" in unnormalized_data
        
        # Check for decomposed state components
        if has_pos and has_gripper:
            # Extract required components and drop unused entries
            pos = unnormalized_data.pop("state.position")
            gripper = unnormalized_data.pop("state.gripper")
            
            # Remove rotation if present
            if "state.rotation" in unnormalized_data:
                unnormalized_data.pop("state.rotation")
            
        elif "state" in unnormalized_data:
            full_state = unnormalized_data.pop("observation.state")

        # Ensure delta_state is preserved
        if "delta_state" not in unnormalized_data:
             unnormalized_data["delta_state"] = normalized_delta_state.cpu()
    ...
    def _load_horizons(self):
        ...
        self._video_delta_indices = self._trim_future_deltas(self._video_delta_indices)
