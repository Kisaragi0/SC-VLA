...
class GR00TTransform(InvertibleModalityTransform):
    ...
    def _apply_vlm_processing(self, batch: dict, state: str) -> BatchFeature:
        """
        Args:
            batch:
                video: [V, T, C, H, W]
    ...
    def _prepare_state(self, data: dict):
        ...
        if self.training:
            ...
            assert state.shape[0] == 1, f"{state.shape=}, {self.state_horizon=}"
    ...
    # progress transforms
    def _prepare_progress(self, data: dict):
        """
        Extracts progress information from data['progress'], 
        pads or truncates to max_progress_dim, and creates a mask.
        Returns (progress, progress_mask, n_progress_tokens).
        """

        if "progress" not in data:
            progress = np.zeros((1, 1))
            progress_mask = np.zeros((1, 1), dtype=bool)
            n_progress_tokens = 1
            return progress, progress_mask, n_progress_tokens

        progress = data["progress"]
        if progress.ndim == 1:
            progress = progress.reshape(-1, 1)

        # Enforce a single temporal token for progress
        assert progress.shape[0] == 1, (
            f"Unexpected progress shape: {progress.shape}, expected (1, D)"
        )

        n_progress_dims = progress.shape[-1]

        if hasattr(self, "max_progress_dim"):
            max_dim = self.max_progress_dim
        else:
            max_dim = 1

        if n_progress_dims > max_dim:
            progress = progress[:, :max_dim]
            n_progress_dims = max_dim
        else:
            progress = np.pad(progress, ((0, 0), (0, max_dim - n_progress_dims)), "constant")

        progress_mask = np.zeros_like(progress, dtype=bool)
        progress_mask[:, :n_progress_dims] = True

        n_progress_tokens = progress.shape[0]
        return progress, progress_mask, n_progress_tokens
    ...
    def apply_single(self, data: dict) -> dict:
        ...
        vlm_outputs = self._apply_vlm_processing(batch_data, 'curr')
        ...
        if self.training:
            ...
            # 4) Prepare Progress
            progress, progress_mask, _ = self._prepare_progress(data)
            transformed_data["progress"] = progress
            transformed_data["progress_mask"] = progress_mask
