...
class LeRobotSingleDataset(Dataset):
    ...
    def apply_random_offset(self, delta_indices, base_index, offset_range=1):
        """
        Apply random temporal offsets to delta indices (except the first one)
        and shift them by a base index to obtain final step indices.
        """
        delta_indices = np.array(delta_indices, dtype=int).copy()

        for i in range(1, len(delta_indices)):
            offset = np.random.randint(-offset_range, offset_range + 1)
            delta_indices[i] = max(1, delta_indices[i] + offset)

            if delta_indices[i] == 0:
                delta_indices[i] = 1
    ...
    def _get_metadata(self, embodiment_tag: EmbodimentTag) -> DatasetMetadata:
        ...
        # 1.3. Progress modalities
        simplified_modality_meta["progress"] = {
            "value": {
                "absolute": True,
                "rotation_type": None,
                "shape": [1],
    ...
    def get_state_or_action(
        ...
        # Get the step indices
        if modality == "state":
            step_indices = self.apply_random_offset(self.delta_indices[key], base_index)
        else:                                    
            step_indices = self.delta_indices[key] + base_index
        ...
        if modality == "progress" and key == "progress.value":
            """
            Compute normalized task progress based on frame indices.
            """
            ...
            # Progress is defined as the normalized frame index within the trajectory
    ...
    def get_data_by_modality(
        ...
        elif modality == "state" or modality == "action" or modality == "progress":
