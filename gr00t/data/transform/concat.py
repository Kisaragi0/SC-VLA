...
class ConcatTransform(InvertibleModalityTransform):
    ...
    training: bool = Field(
        default=True, description="Whether to apply the concat in training mode."
    )
    ...
    # Progress
    progress_concat_order: Optional[list[str]] = Field(
        default=None,
        description="Concatenation order for each progress modality. "
    ...
    def apply(self, data: dict) -> dict:
        ...
        if self.training:            
            if "video.image" in data:
                video = data["video.image"]
                if video.ndim == 4 and video.shape[0] == 2:     # (2, H, W, C)
                    data["video.image"] = video[0:1]

            if "video.wrist_image" in data:
                wrist = data["video.wrist_image"]
                if wrist.ndim == 4 and wrist.shape[0] == 2:
                    data["video.wrist_image"] = wrist[0:1]

            if data["video.image"].shape[0] != 1 or data["video.wrist_image"].shape[0] != 1:
                raise ValueError(f"video.image or video.wrist_image must have exactly 1 frame")
        ...
        # Progress
        if "progress" in grouped_keys:
            progress_keys = grouped_keys["progress"]
            assert all(item in progress_keys for item in self.progress_concat_order), \
                f"keys in progress_concat_order are misspecified, \n{progress_keys=}, \n{self.progress_concat_order=}"

            data["progress"] = torch.cat(
                [data.pop(key) for key in self.progress_concat_order], dim=-1
            )
