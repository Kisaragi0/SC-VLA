...
from gr00t.experiment.data_config import load_data_config, DATA_CONFIG_MAP
...
class ArgsConfig:
    ...
    # "arx5" for real robot and "panda" for maniskill
    ...
    # Model parameters
    base_model_path: str = "Path/to/GR00T-N1.5-3B"
    ...
    embodiment_tag: Literal[tuple(EMBODIMENT_TAG_MAPPING.keys())] = "panda"  # "arx5" for real robot and "panda" for maniskill
    """Embodiment tag to use for training. e.g. 'new_embodiment', 'gr1'"""

    video_backend: Literal["torchcodec", "decord", "torchvision_av"] = "torchvision_av" # torchcodec for real robot and torchvision_av for maniskill
    ...
    train_mode: str = "from_pretrained_with_random_state_encoder"            # "from_pretrained" "from_pretrained_with_random_state_encoder"
...
def reinit_module(module, name=""):
    print(f"[Reinit] Resetting all weights in {name} ...")
    for n, p in module.named_parameters():
        if not p.is_floating_point():
            continue
        if "b" in n or "bias" in n:
            torch.nn.init.zeros_(p)
        else:
            torch.nn.init.normal_(p, mean=0.0, std=0.02)

def reinit_norm_layer(module, name=""):
    print(f"[Reinit] Resetting normalization layer: {name}")

    for n, p in module.named_parameters():
        if p.dtype.is_floating_point:
            if "weight" in n:
                torch.nn.init.ones_(p)   # γ ← 1
                print(f"  - Reset {name}.{n} to ones")
            elif "bias" in n:
                torch.nn.init.zeros_(p) # β ← 0
                print(f"  - Reset {name}.{n} to zeros")
...
def check_model_weights(model, name="model", 
                        ...
                        silent=False):
    """
    Perform a global sanity check on model parameters.
    """
    has_issue = False
    report_lines = []
    ...
    for n, p in model.named_parameters():
        ...
        # ---- Sanity checks ----
        if nan_check and not torch.isfinite(data).all():
            report_lines.append(f"[NaN/Inf] {name}.{n}")
            has_issue = True
        
        elif max_val > max_threshold:
            report_lines.append(f"[Too Large] {name}.{n}: max={max_val:.3e}")
            has_issue = True
        
        elif "b" not in n and max_val == 0:
            report_lines.append(f"[All Zero] {name}.{n}")
            has_issue = True
        
        elif std_val < 1e-8 and max_val > 0:
            report_lines.append(f"[Near Constant] {name}.{n}: std={std_val:.3e}, max={max_val:.3e}")
            has_issue = True
    
    if not silent:
        if has_issue:
            print(f"\n⚠️ [CheckResult] {name}: Found abnormal parameters ↓")
            for line in report_lines:
                print("  ", line)
        else:
            print(f"✅ [CheckResult] {name}: All parameters look healthy.")
...
def main(config: ArgsConfig):
    ...
    # Load model
    if config.train_mode == 'from_pretrained':
    ...
    elif config.train_mode == 'from_pretrained_with_random_state_encoder':
        # # # Random initialization
        model_config = GR00T_N1_5_Config.from_pretrained(config.base_model_path)

        model = GR00T_N1_5.from_pretrained_with_random_state_encoder(
            pretrained_model_name_or_path=config.base_model_path,
    ...
    # Delayed re-initialization to ensure deterministic behavior
    if (not config.resume) and (config.train_mode == 'from_pretrained'):
        reinit_module(model.action_head.delta_state_encoder, "delta_state_encoder")
        reinit_module(model.action_head.delta_state_decoder, "delta_state_decoder")
        reinit_module(model.action_head.progress_encoder,    "progress_encoder")
        reinit_module(model.action_head.progress_decoder,    "progress_decoder")
        reinit_module(model.action_head.future_pred_proj,    "future_pred_proj")
        reinit_module(model.action_head.future_state_proj,    "future_state_proj")

        reinit_norm_layer(model.action_head.align_norm,    "align_norm")

    # Sanity check to catch abnormal weights before training
    if check_model_weights(model.action_head, "action_head"):
        msg = "[ERROR] Abnormal weights detected in action_head — training aborted!"
        print(msg)
